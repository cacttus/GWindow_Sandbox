...
#define V_ENM_STR(xx_) \
  if (r == xx_) { return string_t(#xx_); }
#define V_ENM_STR2(xx_, yy_) \
  if (r == xx_) { return string_t(#yy_); }
#define V_STC_STR(x, y)                                                                       \
  str += Stz " [" + App::toHex((int)off, true) + "][" + off + "]" + std::string(#y) + "\r\n"; \
  off += sizeof(x::y);

#define V_MATCH_ENM(x, y) \
  if ((input) == (x)) { return y; }

VkShaderStageFlagBits VulkanUtils::spvReflectShaderStageFlagBitsToVk(SpvReflectShaderStageFlagBits b) {
...
ShaderStage VulkanUtils::spvReflectShaderStageFlagBits_To_ShaderStage(SpvReflectShaderStageFlagBits input) {
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_VERTEX_BIT, ShaderStage::VertexStage);
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_TESSELLATION_CONTROL_BIT, ShaderStage::TessControlStage);
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, ShaderStage::TessEvalStage);
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_GEOMETRY_BIT, ShaderStage::GeometryStage);
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_FRAGMENT_BIT, ShaderStage::FragmentStage);
  V_MATCH_ENM(SPV_REFLECT_SHADER_STAGE_COMPUTE_BIT, ShaderStage::ComputeStage);

  BRThrowNotImplementedException();
}
VkShaderStageFlagBits VulkanUtils::ShaderStage_to_VkShaderStageFlagBits(ShaderStage input) {
  V_MATCH_ENM(ShaderStage::VertexStage, VK_SHADER_STAGE_VERTEX_BIT);
  V_MATCH_ENM(ShaderStage::TessControlStage, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
  V_MATCH_ENM(ShaderStage::TessEvalStage, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
  V_MATCH_ENM(ShaderStage::GeometryStage, VK_SHADER_STAGE_GEOMETRY_BIT);
  V_MATCH_ENM(ShaderStage::FragmentStage, VK_SHADER_STAGE_FRAGMENT_BIT);
  V_MATCH_ENM(ShaderStage::ComputeStage, VK_SHADER_STAGE_COMPUTE_BIT);
  //V_MATCH_ENM(AllGraphicsStages, VK_SHADER_STAGE_ALL_GRAPHICS);
  V_MATCH_ENM(ShaderStage::AllStages, VK_SHADER_STAGE_ALL);
  //V_MATCH_ENM(RaygenStageNV, VK_SHADER_STAGE_RAYGEN_BIT_NV);
  //V_MATCH_ENM(AnyHitStageNV, VK_SHADER_STAGE_ANY_HIT_BIT_NV);
  //V_MATCH_ENM(ClosestHitStageNV, VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV);
  //V_MATCH_ENM(MissStageNV, VK_SHADER_STAGE_MISS_BIT_NV);
  //V_MATCH_ENM(IntersectionStageNV, VK_SHADER_STAGE_INTERSECTION_BIT_NV);
  //V_MATCH_ENM(CallableStageNV, VK_SHADER_STAGE_CALLABLE_BIT_NV);
  //V_MATCH_ENM(TaskStageNV, VK_SHADER_STAGE_TASK_BIT_NV);
  //V_MATCH_ENM(MeshStageNV, VK_SHADER_STAGE_MESH_BIT_NV);

  BRThrowNotImplementedException();
}

int VulkanUtils::SampleCount_ToInt(MSAA c) {
...
//This is more of a template for future usage of access violations.
string_t VulkanUtils::VkGraphicsPipelineCreateInfo_toString() {
...
string_t VulkanUtils::vkShaderStageFlagBits_toString(VkShaderStageFlagBits r) {
  V_ENM_STR(VK_SHADER_STAGE_VERTEX_BIT);
  V_ENM_STR(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
  V_ENM_STR(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
  V_ENM_STR(VK_SHADER_STAGE_GEOMETRY_BIT);
  V_ENM_STR(VK_SHADER_STAGE_FRAGMENT_BIT);
  V_ENM_STR(VK_SHADER_STAGE_COMPUTE_BIT);
  V_ENM_STR(VK_SHADER_STAGE_ALL_GRAPHICS);
  V_ENM_STR(VK_SHADER_STAGE_ALL);
  V_ENM_STR(VK_SHADER_STAGE_RAYGEN_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_ANY_HIT_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_MISS_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_INTERSECTION_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_CALLABLE_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_TASK_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_MESH_BIT_NV);
  V_ENM_STR(VK_SHADER_STAGE_FLAG_BITS_MAX_ENUM);
  
  BRThrowNotImplementedException();
} 
string_t VulkanUtils::ShaderStage_toString(ShaderStage r) {
  V_ENM_STR(ShaderStage::Unset);
  V_ENM_STR(ShaderStage::VertexStage);
  V_ENM_STR(ShaderStage::TessControlStage);
  V_ENM_STR(ShaderStage::TessEvalStage);
  V_ENM_STR(ShaderStage::GeometryStage); 
  V_ENM_STR(ShaderStage::FragmentStage);
  V_ENM_STR(ShaderStage::ComputeStage);
  // V_ENM_STR(ShaderStage::AllGraphicsStages);
  V_ENM_STR(ShaderStage::AllStages);
  //V_ENM_STR(ShaderStage::RaygenStageNV);
  //V_ENM_STR(ShaderStage::AnyHitStageNV);
  //V_ENM_STR(ShaderStage::ClosestHitStageNV);
  //V_ENM_STR(ShaderStage::MissStageNV);
  //V_ENM_STR(ShaderStage::IntersectionStageNV);
  //V_ENM_STR(ShaderStage::CallableStageNV);
  //V_ENM_STR(ShaderStage::TaskStageNV);
  //V_ENM_STR(ShaderStage::MeshStageNV);
  BRThrowNotImplementedException();
}
string_t VulkanUtils::VkRenderPassBeginInfo_toString() {
  string_t str = "[ hex] [decimal] member_name \r\n";
  size_t off = 0;
...
string_t VulkanUtils::OutputMRT_toString(OutputMRT r) {
...
string_t VulkanUtils::VkDescriptorType_toString(VkDescriptorType r) {
...
string_t VulkanUtils::VkMemoryPropertyFlags_toString(VkMemoryPropertyFlags r) {
...
string_t VulkanUtils::VkColorSpaceKHR_toString(VkColorSpaceKHR r) {
...
string_t VulkanUtils::VkFormat_toString(VkFormat r) {
...
string_t VulkanUtils::VkResult_toString(VkResult r) {
